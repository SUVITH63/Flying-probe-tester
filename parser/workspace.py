...
class WorkspaceValidator:
    ...
    def validate_pad_pair(self, pad_a: Pad, pad_b: Pad, center_y: float = 57.5) -> Tuple[bool, str]:
        """
        Validates if Pad A can be probed by Arm 0 and Pad B can be probed by Arm 1
        without physical collision or crossing the center boundary simultaneously.
        """
        ...
        # Anti-Collision Rule 1: Above center point (Y > 57.5mm), ONLY ONE arm can enter
        if pad_a.y > center_y and pad_b.y > center_y:
            return False, f"Collision Hazard: Both probes ({pad_a.pad_id}, {pad_b.pad_id}) are above center line Y={center_y}mm."

        # Anti-Collision Rule 2: Below center point (Y < 57.5mm), ONLY ONE arm can enter
        if pad_a.y < center_y and pad_b.y < center_y:
            return False, f"Collision Hazard: Both probes ({pad_a.pad_id}, {pad_b.pad_id}) are below center line Y={center_y}mm."

        # Anti-Collision Rule 3: Probe tip proximity margin (minimum 4mm clearance)
        dist = math.sqrt((pad_a.x - pad_b.x)**2 + (pad_a.y - pad_b.y)**2)
        if dist < 4.0:
            return False, f"Proximity Hazard: Probe tips are too close ({dist:.2f}mm < 4.0mm margin)."
